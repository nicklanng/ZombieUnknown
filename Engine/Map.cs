...
namespace Engine
{
    public class Map
    {
        ...
        public void AddLight(Vector2 position, Light light)
        {
            var parentTile = _tiles[(int)position.X, (int)position.Y];
            parentTile.AddLight(light);
        ...
        private void RegenerateLightMap()
        {
            for (var x = 0; x < _width; x++)
            {
                for (var y = 0; y < _height; y++)
                {
                    ...
                    var maxRed = _ambientLight.R;
                    var maxGreen = _ambientLight.G;
                    var maxBlue = _ambientLight.B;

                    foreach (var light in _lights)
                    {
                        var lightPosition = light.Parent.Position;
                        var distance = Vector2.Distance(tilePosition, lightPosition);
                        ...
                        var scalar = 1 - (distance / light.Range);

                        var red = (byte)(light.Color.R * scalar);
                        var green = (byte)(light.Color.G * scalar);
                        var blue = (byte)(light.Color.B * scalar);
                    ...
                    _tiles[x, y].SetLight(new Color(maxRed, maxGreen, maxBlue));
                }
            }
        }
    }
